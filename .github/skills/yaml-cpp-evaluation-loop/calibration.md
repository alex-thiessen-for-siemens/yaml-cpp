...
| --- | --- | --- |
| #1482, empty `YAML::Binary` | `data()` must not form an invalid reference for an empty vector | One focused code change and an empty-value regression |
| #1481, tagged multi-document emission | The emitter must not start a document twice after a tag | A state-transition explanation and repeated tagged-document regression |
| #1476, literal scalar with a tag | An unverified simple key must not pop an unrelated indentation stack entry | A local guard plus malformed-input regression |
...
| #1398, truncated Base64 | Invalid encoded length must not be accepted as complete input | A malformed-input regression without weakening valid decoding |
